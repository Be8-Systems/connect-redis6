 ...
 * MIT Licensed
 * Forked and quick fixed for redis6 by:
 * Oliver Jessner, Boris Sobieski
 * 2022
 */
export default function (session) {
  ...
  class RedisStore extends Store {
      ...
      set(sid, sess, cb = noop) {
          ...
          if (ttl > 0) {
              this.client.set(...args).then(() => cb(args[1]));
          } else {
      ...
      touch(sid, sess, cb = noop) {
          ...
          let key = this.prefix + sid;
          this.client.expire(key, this._getTTL(sess)).then(ret => {
      ...
      destroy(sid, cb = noop) {
          let key = this.prefix + sid;
          this.client.del(key).then(data => cb(data));
      }

      clear(cb = noop) {
          this._getAllKeys((err, keys) => {
              if (err) return cb(err);
              this.client.del(keys).then(data => cb(data));
          });
      }
      ...
      all(cb = noop) {
          ...
          this._getAllKeys((err, keys) => {
              ...
              this.client.mget(keys).then(sessions => {
                  let result;
                  try {
      ...
      _scanKeys(keys = {}, cursor, pattern, count, cb = noop) {
          ...
          this.client.scan(args).then(data => {
